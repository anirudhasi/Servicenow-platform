...
function MultiSelectDropdown({ icon: Icon, label, options, value, onChange, disabled = false }) {
  ...
  const toggle = v => onChange(safeValue.includes(v) ? safeValue.filter(x => x !== v) : [...safeValue, v])
  const toggleAll = () => onChange(safeValue.length === options.length ? [] : [...options])
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        disabled={disabled}
        className={clsx(
          'flex items-center gap-2 px-3 py-2 rounded-lg border font-semibold text-sm transition w-full',
      ...
      >
        ...
        {safeValue.length > 0 && (
          <span className="text-xs bg-brand-600 text-white rounded-full w-5 h-5 flex items-center justify-center shrink-0">
            {safeValue.length}
          </span>
        )}
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-[9999] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl min-w-56 py-1 max-h-64 overflow-y-auto">
          {/* Select All / Clear */}
          <label className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-100 dark:border-slate-700 font-semibold text-xs">
            <input
              type="checkbox"
              checked={safeValue.length === options.length && options.length > 0}
              onChange={toggleAll}
              className="accent-brand-600 w-3.5 h-3.5"
            />
            <span>{safeValue.length === options.length && options.length > 0 ? 'Deselect All' : 'Select All'}</span>
          </label>
          ...
          {options.length === 0 && (
            <p className="px-3 py-3 text-xs text-slate-400 text-center">No options available</p>
          )}
        </div>
      )}
      ...
      {safeValue.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {safeValue.map(v => (
            <span
              key={v}
              className="inline-flex items-center gap-1 px-2 py-0.5 bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 rounded-full text-xs font-semibold"
            >
              {v}
              <button
